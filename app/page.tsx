    ...
    <div className="mt-6 flex gap-6 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent md:grid md:grid-cols-3 md:overflow-x-visible md:pb-0">
      {videos.map((video, index) => (
        <div 
          ...
          className="aspect-video w-[85%] sm:w-[48%] md:w-full bg-zinc-900/50 border border-zinc-800/80 rounded-xl flex flex-col items-center justify-center relative group overflow-hidden snap-center shrink-0 cursor-pointer transition-transform duration-500 ease-out md:scale-100 md:hover:scale-[1.02]"
        >
          ...
          <div className="absolute inset-0 w-full h-full z-0 overflow-hidden">
            ...
            />
            <div className="absolute inset-0 bg-linear-to-t from-zinc-950 via-zinc-950/30 to-transparent opacity-95" />
          </div>
          ...
          {/* Centered Play Button - Gold on mobile, turns gold on desktop hover */}
          <div className="w-12 h-12 rounded-full bg-zinc-950/80 border flex items-center justify-center backdrop-blur-sm transition-all duration-300 z-20 shadow-inner text-amber-400 border-amber-500/30 md:text-zinc-500 md:border-zinc-800 md:group-hover:text-amber-400 md:group-hover:border-amber-500/30">
          ...
          {/* Title Label - Gold text alignment on mobile */}
          <span className="text-[10px] font-mono tracking-widest uppercase mt-3 z-20 transition-colors duration-300 px-4 text-center block w-full truncate drop-shadow-md text-amber-400 md:text-zinc-400 md:group-hover:text-amber-400">
            {video.title}
          </span>
        </div>
      ))}
